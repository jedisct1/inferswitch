...
class ExpertiseBenchmark:
    ...
    def load_expert_configs(self) -> Dict[str, Dict]:
        ...
        domain_config_path = Path(__file__).parent.parent / "examples" / "domain_experts_config.json"
        ...
        custom_config_path = Path(__file__).parent.parent / "examples" / "custom_experts_config.json"
    ...
    def benchmark_model(self, model_name: str, test_cases: List[Dict], expert_configs: Dict[str, Dict]) -> Dict:
        ...
        for config_name, expert_definitions in expert_configs.items():
            ...
            config_test_cases = [tc for tc in test_cases if tc["config_type"] == config_name]
            ...
            for _ in range(3):
                self.classifier.classify_expert(self.convert_to_chat_format("test query"))
            ...
            for i, test_case in enumerate(config_test_cases):
                ...
                confidence = expert_scores.get(predicted_expert, 0.0) if predicted_expert else 0.0
            ...
            for category in set(tc["category"] for tc in config_test_cases):
                ...
                if category_results:
                    category_accuracy = (sum(1 for r in category_results if r["is_correct"]) / len(category_results)) * 100
                    category_confidence = sum(r["confidence"] for r in category_results) / len(category_results)
            ...
            print(f"  Accuracy: {accuracy:.1f}% ({correct_classifications}/{len(config_test_cases)})")
            ...
            for category, metrics in category_metrics.items():
                print(f"    {category}: {metrics['accuracy']:.1f}% accuracy, {metrics['confidence']:.3f} confidence ({metrics['count']} queries)")
    ...
    def print_comparison(self, results: List[Dict]):
        ...
        for result in results:
            ...
            for config_name, config_result in result['configurations'].items():
                print(f"{model_name:<40} {config_name:<20} {config_result['accuracy']:>8.1f}% "
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark MLX models for expertise selection")
