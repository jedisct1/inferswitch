...
def evaluate_single_question(question: str, expected_difficulty: float) -> Dict:
    ...
    return {
        ...
        "correct_bucket": get_difficulty_bucket(predicted_difficulty)
        == get_difficulty_bucket(expected_difficulty),
    }
...
def print_results(results: List[Dict], verbose: bool = False):
    ...
    for bucket in bucket_order:
        if bucket in bucket_results:
            ...
            bucket_rmse = (
                statistics.mean([r["squared_error"] for r in bucket_data])
            ) ** 0.5
            bucket_correct = sum(1 for r in bucket_data if r["correct_bucket"])
            bucket_acc = (bucket_correct / count) * 100

            print(
                f"{bucket:<20} {count:<10} {bucket_mae:<10.3f} {bucket_rmse:<10.3f} {bucket_acc:<10.1f}%"
            )
    ...
    for r in results_sorted[:10]:
        ...
        print(
            f"{question_short:<50} {r['expected']:<10.1f} {r['predicted']:<10.1f} {r['absolute_error']:<10.2f}"
        )
    ...
    if verbose:
        ...
        for r in results:
            print(f"\nQuestion: {r['question']}")
            print(
                f"Expected: {r['expected']}, Predicted: {r['predicted']:.1f}, Error: {r['absolute_error']:.2f}"
            )
...
def main():
    ...
    parser.add_argument(
        "--file",
        default="llm_coding_questions.json",
        help="Path to the questions JSON file",
    )
    parser.add_argument(
        "--limit", type=int, help="Limit number of questions to evaluate"
    )
    parser.add_argument("--verbose", action="store_true", help="Show detailed results")
    parser.add_argument(
        "--bucket", help="Only test questions from specific difficulty bucket"
    )
    ...
    if args.bucket:
        ...
        test_data = [
            q
            for q in test_data
            if get_difficulty_bucket(q["difficulty"]) == args.bucket
        ]
        print(
            f"Filtered to {len(test_data)} questions in bucket '{args.bucket}' (from {original_count} total)"
        )
    ...
    for i, item in enumerate(test_data):
        if i % 10 == 0:
            print(
                f"Progress: {i}/{len(test_data)} ({(i / len(test_data)) * 100:.1f}%)",
                end="\r",
            )
        ...
        json.dump(
            {
                ...
                "metrics": {
                    "mae": statistics.mean([r["absolute_error"] for r in results]),
                    "rmse": (statistics.mean([r["squared_error"] for r in results]))
                    ** 0.5,
                    "bucket_accuracy": (
                        sum(1 for r in results if r["correct_bucket"]) / len(results)
                    )
                    * 100,
