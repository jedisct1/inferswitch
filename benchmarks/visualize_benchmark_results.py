...
def analyze_errors(results: List[Dict]):
    ...
    print("\nError Analysis:")
    print("-" * 70)
    print(f"Correct predictions (±0.5): {len(correct)} ({len(correct)/len(results)*100:.1f}%)")
    print(f"Overestimates: {len(overestimates)} ({len(overestimates)/len(results)*100:.1f}%)")
    print(f"Underestimates: {len(underestimates)} ({len(underestimates)/len(results)*100:.1f}%)")
    ...
    if overestimates:
        ...
        for r, error in overestimates[:5]:
            ...
            print(f"     Expected: {r['expected']}, Predicted: {r['predicted']:.1f}, Error: +{error:.1f}")
...
def analyze_by_category(results: List[Dict]):
    ...
    categories = {
        ...
        "web_development": ["api", "rest", "crud", "jwt", "auth", "react", "javascript"],
    ...
    for category in ["trivial", "documentation", "basic_programming", "web_development", 
                     "system_design", "algorithms", "advanced", "other"]:
        if category in category_stats:
            stats = category_stats[category]
            avg_error = sum(stats["errors"]) / len(stats["errors"])
            max_error = max(stats["errors"])
            
            print(f"{category:<20} {stats['count']:<10} {avg_error:<15.3f} {max_error:<15.3f}")
