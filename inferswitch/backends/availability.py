...
class ModelAvailabilityTracker:
    ...
    def mark_failure(self, model: str):
        ...
        """
        with self._lock:
            disabled_until = datetime.now() + timedelta(seconds=self.disable_duration_seconds)
    ...
    def mark_success(self, model: str):
        ...
        with self._lock:
            if model in self._disabled_models:
                del self._disabled_models[model]
                logger.info(f"Model {model} has been re-enabled due to successful request")
    ...
    def get_disabled_models(self) -> Set[str]:
        ...
        with self._lock:
            ...
            now = datetime.now()
            expired = [
                model for model, until in self._disabled_models.items()
                if now >= until
            ]
