...
class ResponseNormalizer:
    ...
    @staticmethod
    def anthropic_to_openai_messages(anthropic_messages: List[Dict[str, Any]], system: Optional[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def normalize_streaming_chunk(chunk: Dict[str, Any], source_format: str) -> Dict[str, Any]:
        """
        ...
        if source_format == "openai":
            ...
            if chunk.get("choices") and isinstance(chunk.get("choices"), list) and len(chunk["choices"]) > 0 and chunk["choices"][0].get("delta"):
            ...
            # Check for end
            if chunk.get("choices") and isinstance(chunk.get("choices"), list) and len(chunk["choices"]) > 0 and chunk["choices"][0].get("finish_reason"):
                return {
                    "type": "message_delta",
