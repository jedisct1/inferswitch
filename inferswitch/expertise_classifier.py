...
class ExpertClassifier:
    ...
    def load_model(self, model_name: str = "mlx-community/Qwen2.5-Coder-7B-8bit") -> tuple[bool, str]:
        ...
        try:
            logger.debug(f"Attempting to load MLX model for expert classification: {model_name}")
            
            # Load model and tokenizer
            self.model, self.tokenizer = mlx_lm.load(model_name)
            self.model_name = model_name
            
            logger.debug(f"MLX model {model_name} loaded successfully for expert classification")
    ...
    def _clean_query(self, query: str) -> str:
        ...
        cleaned = re.sub(r'</?(?:task|environment_details|slug|name|model)[^>]*>', '', query)
...
Expert:"""
            ...
            try:
                ...
                for defined_expert in self.expert_definitions.keys():
                    if defined_expert.lower() in expert_name or expert_name in defined_expert.lower():
                        logger.debug(f"Partial match: '{expert_name}' -> '{defined_expert}'")
                        return defined_expert
                
                logger.warning(f"MLX returned unrecognized expert: '{expert_name}'. Available: {list(self.expert_definitions.keys())}")
                return None
    ...
    def get_expert_scores(self, chat_messages: List[Dict[str, str]]) -> Dict[str, float]:
        """
                        ...
                        try:
                            ...
                            for defined_expert in self.expert_definitions.keys():
                                if defined_expert.lower() == expert_name.lower():
                                    ...
                                    scores[defined_expert] = min(1.0, max(0.0, score / 5.0))
                                    break
    ...
    def validate_expert_definitions(self) -> Dict[str, Any]:
        ...
        for expert_name, description in self.expert_definitions.items():
            ...
            if not description or not description.strip():
                results["valid"] = False
                results["issues"].append(f"Empty description for expert '{expert_name}'")
            elif len(description.strip()) < 10:
                results["issues"].append(f"Very short description for expert '{expert_name}' (may affect classification quality)")
