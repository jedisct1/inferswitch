...
async def startup_event():
    ...
    if oauth_manager.is_oauth_configured():
        ...
        if not token:
            ...
            logger.info("OAuth configured but no valid token found. Starting interactive authentication...")
            success = await oauth_manager.interactive_oauth_flow()
            if not success:
                logger.warning("OAuth authentication failed. Continuing with API key authentication.")
        else:
...
async def chat_completions_endpoint(
    ...
    if not x_backend and "anthropic" not in model.lower() and "claude" not in model.lower():
...
async def oauth_refresh():
    ...
    try:
        new_token_info = await oauth_manager.refresh_access_token(token_info.refresh_token)
        return {
