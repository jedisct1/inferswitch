...
class MLXModelManager:
    ...
    def rate_query_difficulty(self, chat_messages: List[Dict[str, str]]) -> float:
        ...
        if not self.is_loaded():
            logger.warning("MLX model not loaded, returning default difficulty rating 2.5")
            return 2.5
        ...
        try:
            ...
            logger.debug(f"Extracted user query for difficulty rating: {user_query[:100]}...")
            
            if not user_query:
                logger.warning("No user query found in messages, returning default difficulty 2.5")
                return 2.5
            
            # Clean up the query - remove XML tags and extra whitespace
            import re
            # Remove common XML tags
            cleaned_query = re.sub(r'</?(?:task|environment_details|slug|name|model)[^>]*>', '', user_query)
            ...
            info_keywords = ['what is', 'how does', 'explain', 'tell me', 'describe', 'what are']
            ...
            if is_info_query and not has_how_to:
                ...
                requires_code = any(re.search(pattern, query_lower) for pattern in code_indicators)
                
                # Special case: "How do I" + programming verb almost always requires code
                if has_how_to and any(verb in query_lower for verb in ['print', 'write', 'create', 'implement', 'declare', 'define', 'make', 'build', 'code', 'program', 'read', 'install', 'use', 'set up', 'handle']):
                    requires_code = True
                
                logger.debug(f"Code detection - Query: {query_lower[:50]}... Info: {is_info_query}, How-to: {has_how_to}, Requires code: {requires_code}")
            
            # Check for expert-level indicators
            expert_keywords = [
                'compiler', 'interpreter', 'garbage collector', 'memory allocator',
                'distributed', 'consensus', 'microservice', 'architecture',
                'design.*system', 'build.*from scratch', 'custom.*algorithm',
                'implement.*protocol', 'crdt', 'raft', 'paxos', 'byzantine'
            ]
            
            is_expert_level = any(re.search(keyword, query_lower) for keyword in expert_keywords)
            ...
            if requires_code:
                ...
                if is_expert_level:
                    ...
                    logger.debug(f"Expert-level task detected -> minimum difficulty: {min_difficulty}")
                # Check for specific difficulty 4 patterns - expanded list
                production_keywords = [
                    'jwt', 'oauth', 'api', 'crud', 'authentication', 'docker', 
                    'middleware', 'websocket', 'graphql', 'database schema',
                    'pagination', 'validation', 'error handling', 'deployment',
                    'ci/cd', 'testing', 'webpack', 'file upload', 'rate limit',
                    'async/await', 'promise', 'callback', 'event', 'streaming'
                ]
                if any(word in query_lower for word in production_keywords) or 'rest api' in query_lower or 'react component' in query_lower:
                    min_difficulty = 3.5  # Bias towards 4
                    logger.debug(f"Production-level task detected -> minimum difficulty: {min_difficulty}")
                else:
                    logger.debug(f"Query requires code implementation -> minimum difficulty: {min_difficulty}")
            else:
                # For non-code queries (explanations, simple questions)
                min_difficulty = 0
                logger.debug(f"Query is informational/simple -> minimum difficulty: {min_difficulty}")
            ...
            if requires_code:
                ...
                if 'implement' in query_lower or 'create' in query_lower or 'build' in query_lower:
            ...
            try:
                ...
                if match:
                    ...
                    if requires_code and rating < min_difficulty:
                        logger.debug(f"Model rated {rating}, but enforcing minimum {min_difficulty} for code task")
                ...
                if numbers:
                    ...
                    logger.warning(f"No rating found in response '{clean_response}', using default 2.5")
                    return 2.5
            except Exception:
                return 2.5
