...
class RequestCache:
    ...
    def _extract_cache_key_fields(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'system' in request_data and request_data['system'] is not None:
            ...
            if isinstance(system_content, str):
                ...
                system_content = re.sub(r'(Current Time|Timestamp|Date):\s*[^\n]+\n?', '', system_content, flags=re.IGNORECASE)
            ...
            for msg in request_data['messages']:
                ...
                if 'content' in msg:
                    if isinstance(msg['content'], str):
                        ...
                        import re
                        content = re.sub(r'<environment_details>.*?</environment_details>\s*', '', content, flags=re.DOTALL)
                        content = re.sub(r'(Current Time|Timestamp|Date):\s*[^\n]+\n?', '', content, flags=re.IGNORECASE)
                        ...
                        for content_item in msg['content']:
                            if isinstance(content_item, dict) and content_item.get('type') == 'text':
                                ...
                                if content_item.get('cache_control', {}).get('type') == 'ephemeral':
                                    continue
                                ...
                                if not text.startswith('<environment_details>'):
                                    ...
                                    import re
                                    text = re.sub(r'<environment_details>.*?</environment_details>\s*', '', text, flags=re.DOTALL)
                                    text = re.sub(r'(Current Time|Timestamp|Date):\s*[^\n]+\n?', '', text, flags=re.IGNORECASE)
                                    text = text.strip()
    ...
    def _compute_hash(self, request_data: Dict[str, Any]) -> str:
        ...
        logger.info(f"Computed cache hash: {hash_value[:16]}... for model={cache_fields.get('model', 'unknown')}")
        return hash_value
    ...
    def get(self, request_data: Dict[str, Any]) -> Optional[Any]:
        ...
        with self.lock:
            if cache_key in self.cache:
                ...
                self.hits += 1
                logger.info(f"Cache hit: {cache_key[:8]}... (hits: {self.hits}, misses: {self.misses})")
                return response
    ...
    def set(self, request_data: Dict[str, Any], response: Any) -> None:
        ...
        with self.lock:
            ...
            self.cache.move_to_end(cache_key)
            logger.info(f"Cached response: {cache_key[:8]}... (size: {len(self.cache)})")
