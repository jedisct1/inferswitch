...
def apply_chat_template(chat_messages: List[Dict[str, str]], 
...
def truncate_chat_template_to_fit(chat_messages: List[Dict[str, str]], 
                                  max_context_size: int = 200000,
                                  model_name: str = "claude-3-opus-20240229") -> List[Dict[str, str]]:
    """
    ...
    def calculate_size(messages):
        return sum(len(msg.get('content', '')) + len(msg.get('role', '')) + 50 for msg in messages)
    ...
    if system_size > max_context_size * 0.5:  # Don't let system take more than half
        for msg in truncated_messages:
            if msg.get('role') == 'system':
                ...
                msg['content'] = msg['content'][:max_system_len] + "\n... (system message truncated)"
    ...
    while i >= 1 and current_conv_size < remaining_space:
        ...
        if conversation_messages[i].get('role') == 'assistant' and i > 0:
            ...
            if conversation_messages[i-1].get('role') == 'user':
                pair_size = (len(conversation_messages[i].get('content', '')) + 
                           len(conversation_messages[i-1].get('content', '')) + 100)
...
def remove_oldest_message_pair(chat_messages: List[Dict[str, str]]) -> List[Dict[str, str]]:
    """
    ...
    if first_user_idx + 1 < len(messages) and messages[first_user_idx + 1].get('role') == 'assistant':
