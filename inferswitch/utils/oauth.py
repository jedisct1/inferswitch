...
class OAuthConfig(BaseModel):
    ...
    client_id: str = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"  # Default, can be overridden
...
class OAuthManager:
    ...
    def _load_oauth_config(self) -> OAuthConfig:
        ...
        config_dict = {
            "client_id": oauth_settings.get("client_id", "9d1c250a-e61b-44d9-88ed-5944d1962f5e"),
            "redirect_uri": oauth_settings.get("redirect_uri", "https://console.anthropic.com/oauth/code/callback"),
            "auth_url": oauth_settings.get("auth_url", "https://claude.ai/oauth/authorize"),
            "token_url": oauth_settings.get("token_url", "https://console.anthropic.com/v1/oauth/token"),
            "scopes": oauth_settings.get("scopes", "org:create_api_key user:profile user:inference")
        }
    ...
    async def exchange_code_for_token(self, code: str, code_verifier: str, state: Optional[str] = None) -> TokenInfo:
        ...
        if token_info.expires_in_seconds <= 300:
            if token_info.refresh_token:
                try:
                    ...
                    token_info = await self.refresh_access_token(token_info.refresh_token)
                except Exception as e:
    ...
    async def interactive_oauth_flow(self) -> bool:
        ...
        logger.info("https://console.anthropic.com/oauth/code/callback?code=XXX&state=YYY")
        ...
        if '#' in user_input and not user_input.startswith('http'):
            ...
            logger.info(f"State: {returned_state[:10]}..." if returned_state else "State: None")
        else:
            # Try to parse as URL
            try:
                ...
                if url_code:
                    ...
                    logger.info(f"URL State: {url_state[:10]}..." if url_state else "URL State: None")
                    logger.info("\nIf you see a different authorization code on the page itself,")
                    logger.info("please run the OAuth flow again and provide that code instead.")
        ...
        try:
            ...
            token_info = await self.exchange_code_for_token(auth_code, code_verifier, returned_state)
            
            logger.info("✓ OAuth authentication successful!")
            logger.info(f"✓ Token expires in {token_info.expires_in_seconds:.0f} seconds")
            if token_info.refresh_token:
