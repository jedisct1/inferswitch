...
def rate_query_difficulty_simple(chat_messages: List[Dict[str, str]]) -> float:
    ...
    if any(phrase in query_lower for phrase in ['explain', 'what is', 'describe', 'tell me about']):
        if 'in simple terms' in query_lower or 'layman' in query_lower or 'non-technical' in query_lower:
            return 1.0
        return 2.0
    
    # Check for "how do I" pattern which often indicates code
    if 'how do i' in query_lower or 'how to' in query_lower:
        if any(word in query_lower for word in ['print', 'write', 'create', 'implement', 'declare', 'make']):
            # Advanced code tasks (5)
            if any(word in query_lower for word in ['compiler', 'distributed', 'consensus', 'garbage collector', 'memory allocator', 'microservice', 'architecture']):
                return 5.0
            # Real-world patterns (4)
            elif any(word in query_lower for word in ['api', 'auth', 'jwt', 'oauth', 'database', 'crud', 'middleware', 'docker', 'react']):
                return 4.0
            # Basic programming (3)
            else:
                return 3.0
    ...
    if any(word in query_lower for word in code_indicators):
        # Advanced code tasks (5)
        if any(word in query_lower for word in ['compiler', 'distributed', 'consensus', 'garbage collector', 'memory allocator', 'microservice', 'architecture', 'from scratch']):
            return 5.0
        # Real-world patterns (4)
        elif any(word in query_lower for word in ['api', 'auth', 'jwt', 'oauth', 'database', 'crud', 'middleware', 'docker', 'react']):
            return 4.0
