        ...
        for expert_name, expected_backend in test_cases:
            try:
                backend = router.select_backend(
                    model="test-model", expert_name=expert_name
                )
                ...
                if backend.name == expected_backend:
                    ...
                    print(
                        f"✗ Expert {expert_name}: Expected {expected_backend}, got {backend.name}"
                    )
                    failed += 1
...
def test_config_loading():
    ...
    try:
        ...
        if (
            "ai_researcher" in expert_definitions
            and "web_developer" in expert_definitions
        ):
