...
def test_difficulty_routing():
    ...
    backends = {
        ...
        "all": MockBackend(
            ...
            supported_models=["gpt-3.5-turbo", "gpt-4", "claude-3-haiku-20240307", 
    ...
    backend = router.select_backend("gpt-4", difficulty_rating=2.5)
    assert backend.name == "medium", f"Expected 'medium' (narrower range), got '{backend.name}'"
    ...
    backend = router_no_default.select_backend("claude-3-opus-20240229", difficulty_rating=None)
