    ...
    for queries, expected_expertise in test_cases:
        ...
        for query in queries:
            ...
            if result == expected_expertise:
                ...
                print(
                    f"✗ {expected_expertise}: '{query[:40]}...' -> {result} (expected {expected_expertise})"
                )
        ...
        print(
            f"{expected_expertise.upper()} accuracy: {passed}/{len(queries)} ({accuracy:.1f}%)"
        )
        print()
...
def test_multimodal_detection():
    ...
    print(
        f"Multimodal detection accuracy: {passed}/{len(multimodal_queries)} ({accuracy:.1f}%)"
    )
