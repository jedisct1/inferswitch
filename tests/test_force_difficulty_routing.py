...
def test_force_difficulty_routing():
    ...
    try:
        ...
        assert selected.name == "backend1", f"Expected backend1 (fallback), got {selected.name}"
        print("✓ Without force routing and no difficulty, fallback is used for unknown model")
        ...
        assert selected.name == "backend3", f"Expected backend3 (from difficulty routing), got {selected.name}"
        print("✓ Without force routing, difficulty is still considered in routing priority")
