...
def test_model_availability_tracker():
    ...
    assert not tracker.is_available("claude-3-haiku"), (
        "Model should be disabled after failure"
    )
    assert "claude-3-haiku" in tracker.get_disabled_models(), (
        "Model should be in disabled list"
    )
    print("✓ Model is disabled after failure")

    # Test 3: Other models should still be available
    assert tracker.is_available("claude-3-sonnet"), (
        "Other models should remain available"
    )
    print("✓ Other models remain available")

    # Test 4: Mark success should re-enable
    tracker.mark_success("claude-3-haiku")
    assert tracker.is_available("claude-3-haiku"), (
        "Model should be re-enabled after success"
    )
    ...
    assert tracker.is_available("claude-3-opus"), (
        "Model should be auto-enabled after timeout"
    )
...
def test_router_with_model_lists():
    ...
    with patch.object(
        BackendConfigManager, "get_difficulty_model_mapping"
    ) as mock_diff:
        with patch.object(
            BackendConfigManager, "get_model_provider_mapping"
        ) as mock_prov:
            with patch.object(
                ...
                anthropic_backend = MockBackend(
                    "anthropic", ["claude-3-haiku", "claude-3-sonnet", "claude-3-opus"]
                )
                openai_backend = MockBackend(
                ...
                assert backend.name == "anthropic", (
                    f"Should select anthropic, got {backend.name}"
                )
                assert hasattr(backend, "_difficulty_selected_model"), (
                    "Should have selected model"
                )
                assert backend._difficulty_selected_model == "claude-3-haiku", (
                    "Should select claude-3-haiku"
                )
                ...
                assert backend.name == "openai", (
                    f"Should select openai, got {backend.name}"
                )
                assert backend._difficulty_selected_model == "gpt-3.5-turbo", (
                ...
                try:
                    backend = router.select_backend(
                        "claude-3-haiku", difficulty_rating=0.2
                    )
...
async def test_api_integration():
    ...
    try:
        from inferswitch.api import messages_v2

        print(f"✓ API imports successful: {messages_v2.__name__}")
...
def test_configuration_loading():
    ...
    assert "disable_duration_seconds" in config, (
        "Config should have disable_duration_seconds"
    )
    assert "max_retries" in config, "Config should have max_retries"
    assert config["disable_duration_seconds"] == 300, (
        "Default disable duration should be 300"
    )
    ...
    config = BackendConfigManager.get_model_availability_config()
    assert config["disable_duration_seconds"] == 600, (
