...
def create_large_conversation(num_exchanges=50):
    ...
    for i in range(num_exchanges):
        messages.append(
            {
                "role": "user",
                "content": f"Question {i + 1}: This is a somewhat long user message that contains various details about topic {i + 1}. "
                * 10,
            }
        )
        messages.append(
            {
                "role": "assistant",
                "content": f"Answer {i + 1}: This is a detailed assistant response explaining the answer to question {i + 1}. "
                * 15,
            }
        )
...
large_system_request = {
    "model": "claude-3-opus-20240229",
    "system": "This is an extremely detailed system prompt that contains extensive instructions. "
    * 200,
    "messages": [
...
test_messages = convert_to_chat_template({"messages": create_large_conversation(20)})
...
conversation_ending_with_user = {
    ...
    + [
        {
            "role": "user",
            "content": "This is the final important user question that must be preserved!",
        }
    ],
}
...
print(
    "This would be auto-truncated in log_chat_template() since it exceeds 100,000 chars"
)

# Show what the truncation would look like
if total_size > 100000:
    truncated = truncate_chat_template_to_fit(
        chat_messages, max_context_size=100000, model_name=huge_request["model"]
    )
