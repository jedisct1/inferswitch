...
from main import remove_oldest_message_pair, truncate_chat_template_to_fit, convert_to_chat_template
...
for i in range(20):
    ...
    request["messages"].append({
        "role": "assistant",
        "content": f"Answer {i+1}: Here's a comprehensive explanation of concept {i+1}. " * 5
    })
...
while calculate_size(strategy1_messages) > target_size and len(strategy1_messages) > 1:
    ...
    print(f"  Removal {removals}: {len(strategy1_messages)} messages, {current_size} chars")

print(f"\nResult: Removed {removals} pairs")
print(f"Final: {len(strategy1_messages)} messages, {calculate_size(strategy1_messages)} chars")
...
print(f"Result: {len(strategy2_messages)} messages, {calculate_size(strategy2_messages)} chars")

# Check for truncation notice
has_notice = any('[Previous' in msg['content'] for msg in strategy2_messages if msg['role'] == 'system')
print(f"Truncation notice added: {has_notice}")

# Strategy 3: Hybrid approach
print("\n\nStrategy 3: Hybrid approach")
print("-" * 50)
print("First use bulk truncation for large reduction, then fine-tune with gradual removal")
...
hybrid_messages = truncate_chat_template_to_fit(
    hybrid_messages,
    max_context_size=intermediate_target // 4,
    model_name="gpt-4"
)
print(f"After bulk truncation: {len(hybrid_messages)} messages, {calculate_size(hybrid_messages)} chars")
...
print(f"After fine-tuning ({fine_tune_removals} removals): {len(hybrid_messages)} messages, {calculate_size(hybrid_messages)} chars")

# Comparison
print("\n\nComparison of Strategies")
print("-" * 50)
print(f"Original: {len(chat_messages)} messages, {original_size:,} chars")
print(f"Strategy 1 (gradual): {len(strategy1_messages)} messages, {calculate_size(strategy1_messages)} chars")
print(f"Strategy 2 (bulk): {len(strategy2_messages)} messages, {calculate_size(strategy2_messages)} chars")
print(f"Strategy 3 (hybrid): {len(hybrid_messages)} messages, {calculate_size(hybrid_messages)} chars")

print("\n\nKey Differences:")
print("- remove_oldest_message_pair: Simple, predictable, removes one pair at a time")
print("- truncate_chat_template_to_fit: Intelligent, preserves recent context, bulk operation")
