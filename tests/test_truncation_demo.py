...
def create_conversation(num_exchanges=10):
    ...
    for i in range(num_exchanges):
        messages.append({
            "role": "user",
            "content": f"Question {i+1}: What about topic {i+1}?"
        })
        messages.append({
            "role": "assistant", 
...
for i, msg in enumerate(truncated):
    print(f"  {i+1}. [{msg['role']}] {msg['content'][:60]}{'...' if len(msg['content']) > 60 else ''}")
...
for size in context_sizes:
    ...
    has_notice = any('[Previous' in m.get('content', '') for m in truncated if m['role'] == 'system')
    ...
    if kept_conv > 0:
        ...
        for i in range(len(truncated)-1, -1, -1):
            if truncated[i]['role'] == 'user':
                ...
                break
        if last_user_idx >= 0:
            print(f"  Last user message: \"{truncated[last_user_idx]['content'][:50]}...\"")
...
if total_size > 100000:
    ...
    truncated4 = truncate_chat_template_to_fit(
        ...
        model_name=large_request['model']
    )
