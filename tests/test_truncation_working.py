    ...
    messages.append(
        {
            ...
            + "This is a detailed response. " * 30,
        }
    )
...
messages2 = [
    {
        "role": "system",
        "content": "System instructions: " + "Very detailed instructions. " * 100,
    },
...
for i in range(500):
    huge_messages.append({"role": "user", "content": f"Question {i}: " + "x" * 200})
    huge_messages.append({"role": "assistant", "content": f"Answer {i}: " + "y" * 200})
...
if huge_size > 100000:
    ...
    truncated_api = truncate_chat_template_to_fit(
        huge_messages, max_context_size=100000, model_name="claude-3-opus-20240229"
    )
